...
<script type = "text/javascript">
		...
		function createChart(){
			...
			option = {
				    ...
				    tooltip : {
				        ...
				        /* 	var sAddr = p.indicator;
				        	var dAddr = p.indicator2;
				        	var parmas=[];
				    	...
				    	var myChart = ec.init(document.getElementById('chart'));
					    	myChart.on("click",function(params){
								...
								if(log=="skyeye-tcpflow"){
									...
									if(names.length>1){
										chatTable.parmasChat.push({name:"sip",value:names[0]});
										chatTable.parmasChat.push({name:"dip",value:names[1]});
									}else{
										log = "las-syslog";
									...
									if(names.length>1){
										chatTable.parmasChat.push({name:"sAddr",value:names[0]});
										chatTable.parmasChat.push({name:"dAddr",value:names[1]});
									}else{
										log = "las-syslog";
										titles = ['occurTime','devAddr','category','eventType','sAddr','dAddr','dPort','protocol'];
										chatTable.parmasChat.push({name:"sAddr",value:names[0]});
		...
		function searchLasEs(syslog,lasNet,from,to){
			...
			var date = new Date();
			jsonArray.nodes.push({"category":2,"name":sip,"value":10,id:"0","depth":"0"});
			var lengthNum = 0 ;
			var iN=0;
				...
				searchEs.groupByName(null,index,f,p,from,to,chirdNodeNum,function(arrays){
					...
					$.each(arrays,function(i,v){
						...
						if(num<nodeNum){
							...
							search(field,params,v.term,num+1,node.name);
						}
					});	
					if(iN>=lengthNum){
						createChart();
