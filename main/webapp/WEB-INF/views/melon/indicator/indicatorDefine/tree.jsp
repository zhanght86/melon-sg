...
<div class="col-xs-4 tree-left">
	...
	<mh:tree id="indicatorDefine" 
			 rootId="1"
			 url="indicator/indicatorDefine/find"
			 sortName="order"
			 scriptSelf="false">
	</mh:tree> 
</div>
<div class="col-xs-8 col-right" id ="INDICATOR_CONTAINER" style="margin-top:-20px;margin-bottom:-20px;">
 
</div>

<script type="text/javascript">
$(function(){
	var createUrl = "<c:url value='/indicator/indicatorDefine/create/'/>",
		updateUrl = "<c:url value='/indicator/indicatorDefine/update/'/>",
		removeUrl = "<c:url value='/indicator/indicatorDefine/remove/'/>",
		indicatorDefineForm = $("#INDICATOR_CONTAINER"),
		btnSearch = $("#btnSearch"),
		btnRemove = $("#btnRemove"),
		btnCreate = $("#btnCreate"),
	...
	btnSearch.on("click",function(event){
		indicatorDefineTree.search($.trim(searchName.val()));
		event.preventDefault();
	});
	//菜单资源选择注册
	indicatorDefineTree.onSelect(function(data) {
		var id = data.id;
		indicatorDefineForm.load(updateUrl + id); //加载修改页面 
		//处理删除按钮状态
		if(indicatorDefineTree.isRoot(id)) {
	...
	//默认选中根节点
	indicatorDefineTree.onReady();
	//处理新增按钮
	btnCreate.on("click",function(){
		//请求create页面
		var id = indicatorDefineTree.getSelected();
		indicatorDefineForm.load(createUrl + id); //加载新增页面 
	});
	//处理删除按钮
	btnRemove.on("click",function(event){
		var id = indicatorDefineTree.getSelected(),
			parentId = indicatorDefineTree.getParent(id),
			url = removeUrl + id;
		//确认删除过程
		Dialog.confirm(function() {
			$.post(url, function(data) {
				...
				if(Melon.isSuccess(data)) {
					indicatorDefineTree.remove(id);
					indicatorDefineTree.setSelected(parentId, true);
					indicatorDefineTree.setSelected(id, false);
