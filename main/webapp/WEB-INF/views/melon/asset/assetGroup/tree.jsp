...
<div class="col-xs-8 col-right" id ="ASSETGROUP_CONTAINER">
 
</div>

<script type="text/javascript">
$(function(){
	var createUrl = "<c:url value='/asset/group/create/'/>",
		updateUrl = "<c:url value='/asset/group/update/'/>",
		removeUrl = "<c:url value='/asset/group/remove/'/>",
		assetForm = $("#ASSETGROUP_CONTAINER"),
		btnSearch = $("#btnSearch"),
		btnRemove = $("#btnRemove"),
		btnCreate = $("#btnCreate"),
	...
	btnSearch.on("click",function(event){
		assetGroupTree.search($.trim(searchName.val()));
		event.preventDefault();
	});
	//菜单资源选择注册
	assetGroupTree.onSelect(function(data) {
		var id = data.id;
		assetForm.load(updateUrl + id); //加载修改页面 
		//处理删除按钮状态
		if(assetGroupTree.isRoot(id)) {
	...
	//默认选中根节点
	assetGroupTree.onReady();
	//处理新增按钮
	btnCreate.on("click",function(){
		//请求create页面
		var id = assetGroupTree.getSelected();
		assetForm.load(createUrl + id); //加载新增页面 
	});
	//处理删除按钮
	btnRemove.on("click",function(event){
		var id = assetGroupTree.getSelected(),
			parentId = assetGroupTree.getParent(id),
			url = removeUrl + id;
		//确认删除过程
		Dialog.confirm(function() {
			$.post(url, function(data) {
				...
				if(Melon.isSuccess(data)) {
					assetGroupTree.remove(id);
					assetGroupTree.setSelected(parentId, true);
					assetGroupTree.setSelected(id, false);
