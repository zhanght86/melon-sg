...
(function ($) {
    ...
    $.fn.select2.locales['zh-CN'] = {
        ...
        formatSelectionTooBig: function (limit) { return "你最多只能选择" + limit + "项"; },
        formatLoadMore: function (pageNumber) { return "数据加载中…"; },
...
function AjaxSelect(configs, exConfigs) {
	$.extend(configs, exConfigs);//覆盖配置
	this.view = $("#" + configs.el);//必须是ID
	//默认配置
	this.defaults = {
		placeholder: "请选择",
		...
		tokenSeparators : [","],
		multiple : false,
		maximumSelectionSize: 1,//最多选中的数量
		minimumInputLength: 0,//最少输入字符的数量
		closeOnSelect : true,
	...
	//
	this.setMultiSelect(configs.multiSelect);
	this.setMaxSelectSize(configs.maxSelectSize);
	this.setInitUrl(configs.initUrl);
	this.setFormatResult(configs.formatResult);//设置结果的格式化
	this.setFormatSelection(configs.formatSelection);//设置选项的格式化
	this.setAjax(configs.queryUrl);
	this.setOnceUrl(configs.onceUrl);
}

$.extend(AjaxSelect.prototype, {
	
	/**
	 * 只载入一次, 
 	 * @param {Object} onceUrl
	 */
	setOnceUrl : function(onceUrl) {
		if(!!$.trim(onceUrl)) {
			this.onceUrl = onceUrl;
		}
	},
	
	setMaxSelectSize : function(maxSize) {
		var maxNum = maxSize || 10;//默认只能选择10条
		this.defaults.maximumSelectionSize = maxNum;
	},
	...
	setInitUrl : function(url) {
		if(!!url) {
			this.defaults.initSelection = function(element, callback) {
	...
	setFormatResult : function(formatResult) {
		if($.isFunction(formatResult)) {
			this.defaults.formatResult = formatResult;
		}
	},
	
	setFormatSelection : function(formatSelection) {
		if($.isFunction(formatSelection)) {
			this.defaults.formatSelection = formatSelection;
		}
	},
	
	setAjax : function(queryUrl) {
		if(!!queryUrl) {
			this.defaults.ajax = {
				url : queryUrl,
				dataType : "json",
		...
		if(!!this.onceUrl) {
			$.ajax(this.onceUrl, {
				...
				success : function(data) {
					_this.defaults.query = function(query) {
						query.callback({results : data});
					};
					//
					_this.defaults.initSelection = function(element, callback) {
						var value = $(element).val(),
							i = 0;
		      			if ($.trim(value) !== "") {
		      				for(; i < data.length; i ++) {
					...
					_this.view.select2(_this.defaults);
				}
			});
		} else {
			this.view.select2(this.defaults);
		}
	}
