 ...
 * 日期辅助工具
 */
function DateHelper(configs, exConfigs) {
	$.extend(configs, exConfigs);
	...
	this.setShowSelect(configs.showSelect);
	this.setMinDate(configs.minDate);
	this.setMaxDate(configs.maxDate);
	this.setYearRange(configs.yearRange);
}
...
$.extend(DateHelper.prototype, {
	...
	setShowSelect : function(showSelect) {
		if(!!showSelect) {
	...
	setMaxDate : function(maxDate) {
		if(!!maxDate) {
			this.defaults.maxDate = maxDate;
		}
	},
	
	setYearRange : function(yearRange) {
		if(!!yearRange) {
			this.defaults.yearRange = yearRange;
		}
	},
	
	render : function() {
		this.view.prop("readonly", true);
		this.view.datepicker(this.defaults);
	}
});
 ...
 * 自动完成工具
 */
function AjaxComplete(configs, exConfigs) {
	$.extend(configs, exConfigs);
	this.view = $("#" + configs.el);
	this.defaults = {
		autoFocus: true,
	...
	this.setOnceUrl(configs.onceUrl);//只载入一次,请勿与QueryUrl同时使用
	this.setQueryUrl(configs.queryUrl);
}
$.extend(AjaxComplete.prototype, {
	
	setQueryUrl : function(queryUrl) {
		this.defaults.source = queryUrl;
	},
	...
	render : function() {
		if($.trim(this.onceUrl)) {
			var _this = this;
			$.ajax(this.onceUrl, {
				success : function(data) {
					_this.defaults.source = data;
					_this.view.autocomplete(_this.defaults);
				}
			});
		} else {
			this.view.autocomplete(this.defaults);
		}
	}
