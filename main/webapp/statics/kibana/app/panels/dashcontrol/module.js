...
function(angular, app, _) {
  ...
  module.controller('dashcontrol', function($scope, $http, timer, dashboard, alertSrv) {
    $scope.panelMeta = {
      status  : "Deprecated",
      description : "该面板已经移到导航栏，可通过仪表板配置项来设置它。"
    };
    ...
    $scope.set_default = function() {
      if(dashboard.set_default()) {
        alertSrv.set('Local Default Set',dashboard.current.title+'已被设为本地默认仪表盘','success',5000);
      } else {
        alertSrv.set('Incompatible Browser','浏览器版本过低，不满足此特性','error',5000);
      }
    };

    $scope.purge_default = function() {
      if(dashboard.purge_default()) {
        alertSrv.set('Local Default Clear','本地默认仪表盘已经清除','success',5000);
      } else {
        alertSrv.set('Incompatible Browser','浏览器版本过低，不满足此特性','error',5000);
      }
    };
    ...
    $scope.elasticsearch_save = function(type,ttl) {
      dashboard.elasticsearch_save(
        ...
        function(result) {
        if(!_.isUndefined(result._id)) {
          alertSrv.set('Dashboard Saved','该仪表盘已经保存到Elasticsearch"' +
            result._id + '"','success',5000);
          if(type === 'temp') {
            $scope.share = dashboard.share_link(dashboard.current.title,'temp',result._id);
          }
        } else {
          alertSrv.set('Save failed','仪表盘未能成功保存到Elasticsearch','error',5000);
        }
      });
    };
    ...
    $scope.elasticsearch_delete = function(id) {
      dashboard.elasticsearch_delete(id).then(
        function(result) {
          if(!_.isUndefined(result)) {
            if(result.found) {
              alertSrv.set('Dashboard Deleted',id+'已经删除','success',5000);
              // Find the deleted dashboard in the cached list and remove it
              var toDelete = _.where($scope.elasticsearch.dashboards,{_id:id})[0];
              $scope.elasticsearch.dashboards = _.without($scope.elasticsearch.dashboards,toDelete);
            } else {
              alertSrv.set('Dashboard Not Found','在Elasticsearch未能找到'+id,'warning',5000);
            }
          } else {
            alertSrv.set('Dashboard Not Deleted','删除过程中发生错误','error',5000);
          }
        }
      );
    };
    ...
    $scope.save_gist = function() {
      dashboard.save_gist($scope.gist.title).then(
        ...
        if(!_.isUndefined(link)) {
          $scope.gist.last = link;
          alertSrv.set('Gist saved','已成功导出仪表盘文件'+
            '<a href="'+link+'">'+link+'</a>','success');
        } else {
          alertSrv.set('Save failed','Gist保存失败','error',5000);
        }
      });
    };
    ...
    $scope.gist_dblist = function(id) {
      dashboard.gist_list(id).then(
        ...
        if(files && files.length > 0) {
          ...
          alertSrv.set('Gist Failed','从gist获取仪表盘失败','error',5000);
        }
      });
    };
  });
    ...
    return {
      ...
      link: function(scope) {
        ...
        if (window.File && window.FileReader && window.FileList && window.Blob) {
          ...
          alertSrv.set('Oops','浏览器不支持HTML5部分特性','error');
        }
      }
    };
  });
