...
function (angular, app, _, kbn) {
  ...
  module.controller('trends', function($scope, kbnIndex, querySrv, dashboard, filterSrv) {
    ...
    $scope.panelMeta = {
      ...
      status  : "Beta",
      description : "通过改变时间查询的方法"+
      "例如, 现在时间是下午1点10分, 你的时间可以设置成最近10秒,"+
      "在之前在选择最近1小时, 你的面板中数据结果将会改变为12：00-12：10"
    };
    ...
    $scope.get_data = function(segment,query_id) {
      ...
      if(timeField.length > 1) {
        $scope.panel.error = "时间字段在过滤中必须保存一致";
        return;
      } else if(timeField.length === 0) {
        $scope.panel.error = "一个时间过滤起必须在当前面板的范围内";
        return;
      } else {
