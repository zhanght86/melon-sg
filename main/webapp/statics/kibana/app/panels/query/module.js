...
define([
  ...
  module.controller('query', function($scope, querySrv, $rootScope, dashboard, $q, $modal) {
    ...
    var queryHelpModal = $modal({
      template: Kibana.baseUrl + '/app/panels/query/helpModal.html',
      persist: true,
      show: false,
      scope: $scope,
    });
    ...
    $scope.queryConfig = function(type) {
      return Kibana.baseUrl + "/app/panels/query/editors/"+(type||'lucene')+".html";
    };

    $scope.queryHelpPath = function(type) {
      return Kibana.baseUrl + "/app/panels/query/help/"+(type||'lucene')+".html";
    };
